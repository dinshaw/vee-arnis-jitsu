class Admin::MemorialsController < AdminController
  include ListMethods
  # GET /memorials
  # GET /memorials.xml
  def index
  ...
  def create
    ...
    respond_to do |format|
      if @memorial.save_with_attachments
        flash[:notice] = 'Memorial was successfully created.'
        format.html { redirect_to admin_memorial_url(@memorial) }
  ...
  def update
    ...
    respond_to do |format|
      if @memorial.update_with_attachments(params[:memorial])
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_memorials_url }
      format.xml  { head :ok }
    end
  end
end
