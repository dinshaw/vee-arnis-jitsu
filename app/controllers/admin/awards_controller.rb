class Admin::AwardsController < AdminController
  ...
  # GET /awards.xml
  def index
    @awards = Award.find(:all, :order => :position)
  ...
  def create
    ...
    respond_to do |format|
      if @award.save_with_attachments
        flash[:notice] = 'Award was successfully created.'
        format.html { redirect_to admin_award_url(@award) }
  ...
  def update
    ...
    respond_to do |format|
      if @award.update_with_attachments(params[:award])
