class StoreCategoriesController < ApplicationController
  ...
  def index
    @store_categories = StoreCategory.find(:all)
  ...
  def create
    ...
    respond_to do |format|
      if @store_category.save
        flash[:notice] = 'StoreCategory was successfully created.'
        format.html { redirect_to(@store_category) }
  ...
  def update
    ...
    respond_to do |format|
      if @store_category.update_attributes(params[:store_category])
        flash[:notice] = 'StoreCategory was successfully updated.'
        format.html { redirect_to(@store_category) }
        format.xml  { head :ok }
      else
