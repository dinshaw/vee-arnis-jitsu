class Admin::ArticleCategoriesController < AdminController
  ...
  def index
    @article_categories = ArticleCategory.find(:all, :order => :position)
  ...
  def create
    ...
    respond_to do |format|
      if @article_category.save_with_attachments
        flash[:notice] = 'ArticleCategory was successfully created.'
        format.html { redirect_to admin_article_category_url(@article_category) }
