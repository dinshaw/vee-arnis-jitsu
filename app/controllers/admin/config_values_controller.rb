class Admin::ConfigValuesController < AdminController
  skip_before_filter :do_system_config
  # GET /config_values
  # GET /config_values.xml
  def index
    @config_values = ConfigValue.find(:all, :order => :position)
  ...
  def create
    ...
    respond_to do |format|
      if @config_value.save
        flash[:notice] = 'Config Value was successfully created.'
  ...
  def update
    ...
    respond_to do |format|
      if @config_value.update_attributes(params[:config_value])
        flash[:notice] = 'Config Value was successfully updated.'
        format.html { redirect_to admin_config_values_url }
        format.xml  { head :ok }
      else
